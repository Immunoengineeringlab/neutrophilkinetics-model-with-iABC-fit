function iABC_mice(group_ind, n_rlz, cut_off, n_nest, n_bins, alpha)

%% information on model
indexing;

%% information on inital guess of the range of parameters for model
[lb, ub] = initial_guess();
...
%     toc; tic; % to show time used during the iterations
    ...
    err_vec = calc_error(PARAMETER);
    % err_vec: stores the deviation between prediction (according to
    % PARAMETER) and data. Data is indirectly mediated by group_ind and
    % "information in calc_error" and group_ind. 
...
%     disp([size(Post_pdf), size(PriorDist)]);
    ...
    save(['session_main_', Group{1, group_ind}, '.mat'])
...
% saving the session
save(['session_main_LH_EDUTOTAL_SALINE', '.mat'],...
